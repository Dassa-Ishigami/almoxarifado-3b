...
test('sistema deve barrar a movimentação de saída se a quantidade retirada for maior que o estoque', function() {
    ...
    $movimentoMock = new Movimentacao([
        'quantidade' => 10,
        'tipo' => 's',
    ]);
...
// 2.0 Teste de subtração (simula o afterCreate para saída)
test('sistema deve diminuir o estoque corretamente após uma saída autorizada', function () {
    // Cenário: Estoque inicial de 10, saindo 3
    $produtoMock = new Produto(['estoque' => 10]);
    $movimentoMock = new Movimentacao(['quantidade' => 3, 'tipo' => 's']);
...
test('sistema deve aumentar o estoque corretamente apos uma entrada com sucesso', function () {
    ...
    $movimentoMock = new Movimentacao(['quantidade' => 8, 'tipo' => 'e']);
